...
const requestSsoTokenFromExtension = () => {
...
export const api = () => {
     ...
     * @returns JWT Token.
     */
    const exchangeTokenAndSetAuth = async (oneTimeToken) => {
        try {
            ...
            if (!jwtToken)
                throw new Error("API не повернув JWT.");
    ...
    // -------------------------------------------------------------------------
    const fetchData = async (url, initData, options = {}) => {
        ...
        const executeRequest = async (jwt) => {
            const headers = {
