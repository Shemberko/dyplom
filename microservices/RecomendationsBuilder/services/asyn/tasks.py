...
def build_update_query() -> str:
    ...
    WITH u, V_p, W_new,
         COALESCE(u.sumWeightedEmbedding, [x IN range(0, size(V_p)-1) | 0.0]) AS Sum_old,
         COALESCE(u.totalWeight, 0.0) AS TotalW_old
    ...
    SET u.sumWeightedEmbedding = Sum_new,
        u.totalWeight = TotalW_new,
        u.embedding = CASE 
...
# ( ... validate_embedding_vector та compute_incremental_update без змін ... )
...
def apply_incremental_update_via_python(neo4j_client: Any, user_id: str, page_element_id: str, active_time: float) -> Any:
    ...
    MATCH (u:User {id: $user_id})
    MATCH (p:Page) WHERE elementId(p) = $page_element_id
    RETURN u.sumWeightedEmbedding AS sumWeighted, u.totalWeight AS totalWeight, p.structuralEmbedding AS page_emb
    """
    try:
...
def recompute_user_from_visits_via_python(neo4j_client: Any, user_id: str) -> Any:
    ...
    WRITE_Q = """
    MATCH (u:User {id: $user_id})
    SET u.sumWeightedEmbedding = $sum_weighted,
...
# ==============================================================================
# КОД ДЛЯ TEXT EMBEDDINGS (Виправлено згідно JsonMessageProcessor)
# ==============================================================================

def build_text_update_query() -> str:
    """
    Cypher query для інкрементального оновлення ТЕКСТОВОГО профілю користувача.
    """
    return """
    MATCH (u:User {id: $user_id})
    MATCH (p:Page) WHERE elementId(p) = $page_element_id
    WITH u, p.text_embedding AS V_p, toFloat($active_time) AS W_new 
    WHERE V_p IS NOT NULL AND size(V_p) > 0
    WITH u, V_p, W_new,
         COALESCE(u.sumWeightedTextEmbedding, [x IN range(0, size(V_p)-1) | 0.0]) AS Sum_old,
         COALESCE(u.totalTextWeight, 0.0) AS TotalW_old
    WITH u, [i IN range(0, size(V_p)-1) | V_p[i] * W_new] AS V_new_weighted, Sum_old, TotalW_old, W_new
    WITH u, [i IN range(0, size(Sum_old)-1) | Sum_old[i] + V_new_weighted[i]] AS Sum_new,
         TotalW_old + W_new AS TotalW_new
    SET u.sumWeightedTextEmbedding = Sum_new,
        u.totalTextWeight = TotalW_new,
        u.textEmbedding = CASE 
            WHEN TotalW_new = 0.0 THEN Sum_new 
            ELSE [i IN range(0, size(Sum_new)-1) | Sum_new[i] / TotalW_new] 
        END
    RETURN u.id
...
def run_text_update_cypher(neo4j_client: Any, user_id: str, page_element_id: str, active_time: float) -> Any:
    """
    Виконує build_text_update_query() через neo4j_client.run_query().
    """
    q = build_text_update_query()
    results = neo4j_client.run_query(q, {'user_id': user_id, 'page_element_id': page_element_id, 'active_time': active_time})
    return results[0] if results else None


def apply_incremental_text_update_via_python(neo4j_client: Any, user_id: str, page_element_id: str, active_time: float) -> Any:
    """
    Python‑варіант оновлення для ТЕКСТОВОГО профілю:
    """
    SELECT_Q = """
    MATCH (u:User {id: $user_id})
    MATCH (p:Page) WHERE elementId(p) = $page_element_id
    RETURN u.sumWeightedTextEmbedding AS sumWeighted, u.totalTextWeight AS totalWeight, p.text_embedding AS page_emb
    """
    try:
        res_list = neo4j_client.run_query(SELECT_Q, {'user_id': user_id, 'page_element_id': page_element_id})
        if not res_list:
        ...
        print(f"Error reading for python (text) update: {e}")
        return None
    ...
    try:
        ...
        print(f"Error writing incremental (text) update: {e}")
        traceback.print_exc()
        return None


@app.task(bind=True, max_retries=3)
def update_user_text_embedding_incrementally(self, user_id: str, page_element_id: str, active_time: float):
    """
    Celery task: інкрементальне оновлення ТЕКСТОВОГО профілю користувача.
    """
    try:
        try:
            res = run_text_update_cypher(neo4j_client, user_id, page_element_id, active_time)
            return f"User {user_id} TEXT profile updated via Cypher."
        except Exception as e_cy:
            print(f"Cypher (text) update failed, falling back to Python update: {e_cy}")
            traceback.print_exc()
            res_py = apply_incremental_text_update_via_python(neo4j_client, user_id, page_element_id, active_time)
            if res_py is None:
                raise RuntimeError("Both cypher and python (text) update failed")
            return f"User {user_id} TEXT profile updated via Python."
    except Exception as exc:
        raise self.retry(exc=exc, countdown=10)
